...
from app import crud, models as app_models
...
# Backward-compatible aliases for tests and local patch points that still
# reference the older private helper names from this router.
_normalize_web_url = normalize_web_url
_fetch_console_status = fetch_console_status
_fetch_console_status_map = fetch_console_status_map
...
class TmuxControlCreditsItemOut(BaseModel):
    connector_id: int
    connector_name: str = ""
    session_name: str = ""
    display_name: str = ""
...
class TmuxControlCreditsResponse(BaseModel):
    items: list[TmuxControlCreditsItemOut]
    count: int
    needs_billing: int = 0
    needs_reauth: int = 0
    downgrade_candidates: int = 0
...
class TmuxControlAuditResponse(BaseModel):
    items: list[TmuxControlAuditEventOut]
    count: int
...
def _web_token(connector: Optional[Connector]) -> str:
    if not connector:
        return ""
    cfg = dict(connector.config or {})
...
def _collector_url(connector: Optional[Connector]) -> str:
    if not connector:
        return ""
    cfg = dict(connector.config or {})
    return str(cfg.get("collector_url") or "").strip() or _web_url(connector)
        ...
        records.append(
            {
                ...
                "web_token": _web_token(connector),
                "collector_url": _collector_url(connector),
            }
        )
    return records
...
def _console_action_url(
    web_url: str, action_path: str, *, access_token: str = ""
) -> str:
...
def _post_console_action(
    web_url: str, action_path: str, *, access_token: str = "", timeout: float = 8.0
) -> dict[str, Any]:
    action_url = _console_action_url(web_url, action_path, access_token=access_token)
    if not action_url:
        raise HTTPException(
    ...
    try:
        ...
        if isinstance(
            exc, (URLError, TimeoutError, OSError, ValueError, json.JSONDecodeError)
        ):
            raise HTTPException(
...
def _record_web_token(record: dict[str, Any]) -> str:
    return str(record.get("web_token") or "").strip()


def _record_collector_url(record: dict[str, Any]) -> str:
    return (
        str(record.get("collector_url") or "").strip()
        or str(record.get("web_url") or "").strip()
    )


async def _fetch_record_status_map(
    records: list[dict[str, Any]],
) -> dict[tuple[str, str], dict[str, Any]]:
    targets: list[tuple[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for record in records:
        web_url = _record_collector_url(record)
        if not web_url:
            continue
        key = (web_url, _record_web_token(record))
        if key in seen:
            continue
        seen.add(key)
        targets.append(key)
    if not targets:
        return {}
    if all(not web_token for _, web_token in targets):
        legacy = await fetch_console_status_map([web_url for web_url, _ in targets])
        return {
            (web_url, ""): legacy.get(web_url, fetch_console_status(""))
            for web_url, _ in targets
        }
    results = await asyncio.gather(
        *[
            asyncio.to_thread(fetch_console_status, web_url, access_token=web_token)
            for web_url, web_token in targets
        ],
        return_exceptions=True,
    )
    mapping: dict[tuple[str, str], dict[str, Any]] = {}
    for key, result in zip(targets, results, strict=False):
        if isinstance(result, Exception):
            mapping[key] = fetch_console_status("")
            continue
        mapping[key] = result
    return mapping
...
def _status_for_record(
    status_map: dict[tuple[str, str], dict[str, Any]], record: dict[str, Any]
) -> dict[str, Any]:
    web_url = _record_collector_url(record)
    web_token = _record_web_token(record)
    return status_map.get((web_url, web_token), fetch_console_status(""))
...
async def list_control_sessions(
    ...
    status_map = await _fetch_record_status_map(
        [record for record in records if bool(record.get("running"))]
    )
    items = [
        TmuxControlSessionOut(
            ...
            status_available=bool(
                _status_for_record(status_map, record).get("reachable")
            ),
            status_message=str(
                _status_for_record(status_map, record).get("status_message") or ""
            ),
            auth_required=bool(
                _status_for_record(status_map, record).get("auth_required")
            ),
            auth_mode=str(
                _status_for_record(status_map, record).get("auth_mode") or ""
            ),
            auth_summary=str(
                _status_for_record(status_map, record).get("auth_summary") or ""
            ),
            auth_verification_url=str(
                _status_for_record(status_map, record).get("auth_verification_url")
                or ""
            ),
            auth_device_code=str(
                _status_for_record(status_map, record).get("auth_device_code") or ""
            ),
        )
        for record in records
...
async def list_control_ops(
    ...
    status_map = await _fetch_record_status_map(records)
    items: list[TmuxControlOpsItemOut] = []

    for record in records:
        web_url = str(record.get("web_url") or "")
        status = _status_for_record(status_map, record)
        state, state_label, state_detail = _derive_ops_state(record, status)
        credit_snapshot = None
        connector_id = record.get("connector_id")
        if connector_id:
            credit_snapshot = await fleet_credit_monitor.get_snapshot(int(connector_id))
        assessment = classify_console_credit_assessment(status)
        issue_code = (
            credit_snapshot.issue_code if credit_snapshot else assessment.issue_code
        )
        issue_label = (
            credit_snapshot.issue_label if credit_snapshot else assessment.issue_label
        )
        issue_summary = (
            credit_snapshot.issue_summary
            if credit_snapshot
            else assessment.issue_summary
        )
        billing_url = (
            credit_snapshot.billing_url if credit_snapshot else assessment.billing_url
        )
        limits_url = (
            credit_snapshot.limits_url if credit_snapshot else assessment.limits_url
        )
        recommended_speed = (
            credit_snapshot.recommended_speed
            if credit_snapshot
            else assessment.recommended_speed
        )
        recommended_speed_reason = (
            credit_snapshot.recommended_speed_reason
            if credit_snapshot
            else assessment.recommended_speed_reason
        )
        items.append(
            TmuxControlOpsItemOut(
                ...
                recommended_speed_reason=recommended_speed_reason,
    ...
    return TmuxControlOpsResponse(
        ...
        needs_billing=sum(1 for item in items if item.issue_code == "needs_billing"),
        needs_reauth=sum(1 for item in items if item.issue_code == "needs_reauth"),
        downgrade_candidates=sum(1 for item in items if item.recommended_speed),
    )


@router.get("/control/credits", response_model=TmuxControlCreditsResponse)
async def list_control_credits(
    socket_path: str = Query("", max_length=512),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    connector_ids = [
        int(record.get("connector_id"))
        for record in records
        if record.get("connector_id") is not None and bool(record.get("managed"))
    ]
    snapshots = await fleet_credit_monitor.get_items(connector_ids)
    ...
    for record in records:
        ...
        if connector_id is None:
            continue
        snap = snapshot_map.get(int(connector_id))
        if not snap:
            continue
        items.append(
            TmuxControlCreditsItemOut(
                connector_id=snap.connector_id,
                connector_name=snap.connector_name,
                session_name=str(record.get("session_name") or ""),
                display_name=str(record.get("connector_name") or "")
                or str(record.get("session_name") or ""),
                web_url=snap.web_url,
                issue_code=snap.issue_code,
                issue_label=snap.issue_label,
                issue_summary=snap.issue_summary,
                billing_url=snap.billing_url,
                limits_url=snap.limits_url,
                chat_model=snap.chat_model,
                default_speed=snap.default_speed,
                recommended_speed=snap.recommended_speed,
                recommended_speed_reason=snap.recommended_speed_reason,
                auth_required=snap.auth_required,
        ...
        for record in records:
            ...
            items.append(
                TmuxControlCreditsItemOut(
                    ...
                    billing_url=assessment.billing_url,
                    limits_url=assessment.limits_url,
                    chat_model=str(status.get("chat_model") or ""),
                    default_speed=str(status.get("default_speed") or ""),
                    ...
                    usage_successful_turns=int(
                        status.get("usage_successful_turns") or 0
                    ),
    ...
    items.sort(
        key=lambda item: (
            ...
            -int(item.usage_window_total_tokens or 0),
            item.display_name.lower(),
        )
    )
    summary = await fleet_credit_monitor.get_summary(connector_ids)
    if not summary.get("count") and items:
        summary = {
            "count": len(items),
            "needs_billing": sum(
            ...
            "usage_turns": sum(int(item.usage_turns or 0) for item in items),
            "usage_input_tokens": sum(
                int(item.usage_input_tokens or 0) for item in items
            ),
            "usage_cached_input_tokens": sum(
                int(item.usage_cached_input_tokens or 0) for item in items
            ),
            "usage_output_tokens": sum(
                int(item.usage_output_tokens or 0) for item in items
            ),
            "usage_total_tokens": sum(
                int(item.usage_total_tokens or 0) for item in items
            ),
            "usage_window_turns": sum(
                int(item.usage_window_turns or 0) for item in items
            ),
            "usage_window_input_tokens": sum(
                int(item.usage_window_input_tokens or 0) for item in items
            ),
            "usage_window_cached_input_tokens": sum(
                int(item.usage_window_cached_input_tokens or 0) for item in items
            ),
            "usage_window_output_tokens": sum(
                int(item.usage_window_output_tokens or 0) for item in items
            ),
            "usage_window_total_tokens": sum(
                int(item.usage_window_total_tokens or 0) for item in items
            ),
...
@router.get("/control/audit", response_model=TmuxControlAuditResponse)
async def list_control_audit(
    ...
    query = db.query(app_models.ConsoleAuditEvent).filter(
        app_models.ConsoleAuditEvent.user_id == current_user.id
    )
    if connector_id is not None:
        query = query.filter(app_models.ConsoleAuditEvent.connector_id == connector_id)
    clean_session = str(session or "").strip()
    if clean_session:
        query = query.filter(app_models.ConsoleAuditEvent.session_name == clean_session)
    clean_event_type = str(event_type or "").strip().lower()
    if clean_event_type:
        query = query.filter(
            app_models.ConsoleAuditEvent.event_type == clean_event_type
        )
    clean_severity = str(severity or "").strip().lower()
    if clean_severity:
        query = query.filter(app_models.ConsoleAuditEvent.severity == clean_severity)

    items = (
        query.order_by(
            app_models.ConsoleAuditEvent.event_at.desc(),
            app_models.ConsoleAuditEvent.id.desc(),
        )
        .limit(limit)
        .all()
    )
    connector_count = len(
        {int(item.connector_id) for item in items if item.connector_id}
    )
    return TmuxControlAuditResponse(
        items=[
            TmuxControlAuditEventOut(
                id=int(item.id),
                connector_id=int(item.connector_id),
                connector_name=str(item.connector_name or ""),
                session_name=str(item.session_name or ""),
                agent_name=str(item.agent_name or ""),
                host_name=str(item.host_name or ""),
                source_event_id=str(item.source_event_id or ""),
                event_type=str(item.event_type or ""),
                severity=str(item.severity or "info"),
                actor_type=str(item.actor_type or "system"),
                actor_ip=str(item.actor_ip or ""),
                thread_id=str(item.thread_id or ""),
...
async def set_control_web_url(
    ...
    web_url = normalize_web_url(payload.web_url)
    cfg = dict(connector.config or {})
    if web_url:
        cfg["web_url"] = web_url
    else:
...
async def start_control_auth_device(
    ...
    collector_url = _collector_url(connector)
    web_token = _web_token(connector)
    remote = _post_console_action(
        collector_url,
        "/api/auth/device",
        **({"access_token": web_token} if web_token else {}),
    )
    snapshot = (
...
async def load_tmux_profile(
    ...
    for item in items:
        ...
        if "web_url" in item:
            item_web_url = normalize_web_url(str(item.get("web_url") or ""))
            cfg = dict(connector.config or {})
