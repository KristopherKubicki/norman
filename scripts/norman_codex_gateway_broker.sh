...
readonly BROKER_IDENTITY_FILE="${NORMAN_CODEX_GATEWAY_BROKER_IDENTITY_FILE:-$HOME/.ssh/id_ed25519_netops_codex}"
...
if [[ ! -r "$BROKER_IDENTITY_FILE" ]]; then
  echo "Norman Codex gateway broker identity is unavailable." >&2
  exit 1
fi
