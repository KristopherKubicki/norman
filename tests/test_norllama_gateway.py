...
def test_frontdoor_urls_do_not_duplicate_v1_prefix():
    ...
    assert (
        gateway._activity_url("https://llm.home.arpa/v1", 25, tool_only=True)
        == "https://llm.home.arpa/v1/activity?limit=25&tool_only=1"
    )
...
def test_fetch_tool_activity_filters_probe_noise(monkeypatch):
    ...
    assert calls[0][0] == "https://llm.home.arpa/v1/activity?limit=200&tool_only=1"
    assert calls[0][1]["Authorization"] == "Bearer token"
    ...
    assert payload["tool_only_source"] is True


def test_fetch_tool_activity_falls_back_for_older_gateways(monkeypatch):
    calls = []

    def fake_get(url, headers, timeout, verify):
        calls.append(url)
        if "tool_only=1" in url:
            raise gateway.requests.ConnectionError("old gateway rejected query")
        return FakeResponse(
            {
                "count": 1,
                "items": [
    ...
    assert calls == [
        "https://llm.home.arpa/v1/activity?limit=25&tool_only=1",
        "https://llm.home.arpa/v1/activity?limit=25",
    ]
    assert payload["tool_call_count"] == 1
    assert payload["tool_only_source"] is False
